            ...
            if (deletingListResponseModel.getResult() != 0) {
                ...
                if (tokensToRemove == null || tokensToRemove.isEmpty()) {
                    ...
                    for (final String token : tokensToRemove) {
                        try {
                            MainActivity.addLog(mUserModel.getLogin(), Constants.LogType.DATABASE, Constants.LogObject.QUESTIONNAIRE, mBaseActivity.getString(R.string.delete_quiz_by_token), Constants.LogResult.SENT, mBaseActivity.getString(R.string.set_sent_quiz_status), null);
...
//                            MainActivity.getStaticDao().deleteQuestionnaireByToken(token);
                            ...
                            MainActivity.addLog(mUserModel.getLogin(), Constants.LogType.DATABASE, Constants.LogObject.QUESTIONNAIRE, mBaseActivity.getString(R.string.delete_quiz_by_token), Constants.LogResult.ERROR, mBaseActivity.getString(R.string.db_save_error), e.getMessage());
                        }
                    }
